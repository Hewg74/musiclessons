  ...
  exercises: [
...

    {
      ...
      steps: [
        ...
        { text: "Turn the drone back on. Sing D again — feel it against the root. Now a body check: where does D resonate in your body? It's different from A (which sits deep, grounded, like a foundation) and different from E (which feels higher, more open). D has its own address — maybe it's a restless buzz in the chest, or a pull forward. Find where D lives physically. This body-map helps you FIND D later without playing it first.", why: "Somatic mapping gives audiation a physical anchor. When you know D lives in a specific place in your body, you have TWO ways to find it: by ear (the interval from the drone) and by feel (the body location). Two pathways are more reliable than one." },
...

    {
      ...
      steps: [
        ...
        { text: "Body check: where does G resonate? It's different from D (restless, middle) and different from A (deep, grounded). G has its own address — probably higher in the body, maybe a forward pull, a brightness in the mask of the face. Find G's home. Now you have body-maps for A, D, and G — three landmarks on your internal landscape.", why: "Building body-maps for each new note creates a physical vocabulary alongside the sonic one. When you know where G lives in your body, you can find it through two channels: ear (interval from drone) and body (physical sensation). This redundancy makes audiation more reliable." },
    ...
    },
    {
      ...
      steps: [
        ...
        { text: "Autopilot check — pause and ask yourself: am I hearing each note a split second before it comes out, or has my voice taken over on cruise control? If you're on autopilot, slow down. Close your eyes. Hear the NEXT note in your head — its color, its weight — then sing it. Audiation keeps you composing, not just noodling.", why: "Extended improvisation triggers a well-documented autopilot trap: the voice repeats comfortable patterns while the ear checks out (Aarhus 2021 fMRI studies show reduced auditory cortex activation during 'zoned-out' improv). The pause-and-listen reset re-engages your inner ear and keeps each note intentional." },
...
,
...

    {
      ...
      steps: [
        ...
        { text: "Building the palette one note at a time: drone on A. Play each pentatonic note one at a time — A, C, D, E, G. For EACH note: play it on guitar, absorb it for 3 seconds against the drone, mute the guitar, hold the note internally against the still-playing drone for 3 seconds, re-sing it, check with the pitch detector. You're grounding each note in the drone anchor before storing it internally. Take your time — this is one full pass through your palette.", why: "This systematic pass maps each note's interval character against the drone. A is unison (easy). C is minor 3rd (blue). D is 4th (restless). E is 5th (open). G is minor 7th (reaching). By grounding each note in the anchor before internalizing it, you're building an internal palette where every note has a KNOWN distance from home. That distance is your map." },
