...
export const level1 = {
  ...
  exercises: [
...

    {
      ...
      steps: [
        { text: "Start on the low E string, 5th fret — that's A. Play it, say 'A' out loud. Move to the 8th fret — that's C. Play it, say 'C.' These two notes live on one string. Do this 4 times, saying each note name.", why: "Saying note names out loud while playing creates three neural pathways simultaneously — auditory, motor, and linguistic. Edwin Gordon's audiation research shows this triples retention compared to silent playing. Most guitarists learn shapes without names, which means they can play the pattern but can't hear it internally." },
        { text: "Continue string by string: A string has D (5th fret) and E (7th fret). D string has G (5th fret) and A (7th fret). G string: B string: high E string. Say every note name as you play it. Take your time — accuracy over speed.", why: "Building the shape string by string makes it manageable. By the time you reach the high E string, your fingers have traced a map across the neck. This map is the same one Mark Speer navigates every night." },
        ...
        { text: "Play ascending and descending 4 times in a row without a wrong note. If you hit a wrong fret, restart the count from zero. No skipping. No fudging.", why: "The '4 clean passes with reset' method is from motor learning research — it ensures the correct movement pattern is the one your brain consolidates during sleep. One sloppy pass can undo three clean ones." },
        { text: "Close your eyes and play it once, ascending and descending. If you can do this, the shape is in your fingers, not just your memory.", why: "Eyes-closed playing proves your hands have mapped the fretboard spatially. This is how scales become second nature — your fingers know the distances between frets the way your feet know the stairs in your house." }
      ],
...

    {
      ...
      steps: [
        ...
        { text: "C: index on B string fret 1, middle on D string fret 2, ring on A string fret 3. Strum strings 5 through 1. Now look at your hand — notice that your ring finger is on the SAME fret and string as it was for Am? That's your anchor finger. When switching Am to C, the ring finger stays planted. Only the other fingers move.", why: "The anchor finger concept is the single biggest shortcut for chord transitions. Instead of lifting all fingers and rebuilding the shape from scratch, you keep one finger planted and the others reorganize around it. Am→C shares the ring finger. This turns a three-finger move into a two-finger move." },
      ...
      wrong: "The most common mistake is pressing too far from the fret wire — your fingertip should be just behind the metal fret, not in the middle of the space between frets. If a string buzzes, move your finger closer to the fret. If a string is muted, check that another finger isn't accidentally touching it. Curl your fingers so the fingertips press straight down, not flat across neighboring strings.",
      sarah: "Gene, these five chords unlock your entire Level 1 song list. DOPE LEMON's Dope & Smoke is Am-D-G. Father John Misty's Real Love Baby is D-Em-G. The Polarity's Son of a Beach is F-G-C-Am (you'll get F later — for now, substitute Am and it still sounds great). Yasawa Group's Pattymanajaro is Am-G-C-Em. Five shapes, four songs, zero barre chords needed.",
...

    {
      ...
      feel: "This should feel like a competitive game against yourself — you're trying to beat your own high score. The excitement of watching your count climb over days is genuinely addictive. When a pair hits 30+ changes per minute, the transition feels invisible.",
      ...
      sarah: "Gene, this drill is boring and magical at the same time. It's boring because it's just two chords back and forth. It's magical because the results are undeniable — you'll feel the transitions getting smoother day by day. The Am-D-G progression in DOPE LEMON's Dope & Smoke will go from clunky to buttery in about two weeks of daily one-minute changes. Trust the grind.",
...

    {
      ...
      steps: [
        { text: "Pick the open A string in steady quarter notes — one per metronome click. Let each note ring until the next one. Listen to how it locks with the drone. This is your home base — one note, steady pulse, zero decisions.", why: "Starting with the simplest possible musical act — one note, steady rhythm — strips away all complexity. From here, every variation you add is a conscious creative choice, not a scramble for notes." },
        ...
        { text: "Add palm muting: rest the edge of your picking hand lightly on the strings near the bridge. The A string becomes a thuddy, percussive thump instead of an open ring. Alternate: 2 muted hits, 2 open hits. Feel the contrast between tight (muted) and open (ringing).", why: "Palm muting adds a timbral dimension — now you have THREE variables on one note: rhythm, dynamics, and tone (muted vs. open). Three variables on one note is more creative freedom than most beginners ever discover." },
...

    {
      ...
      levelUp: "Play the Am blues scale (A-C-D-Eb-E-G-A) ascending and descending 4 times cleanly at 60 BPM. Can identify the blue note by ear when you hear it."
    },
...

    {
      ...
      steps: [
        { text: "Place your ring finger on the B string, 8th fret (G note). Now push the string upward (toward the ceiling) while keeping pressure on the fret. The pitch should rise. Push until it sounds like the note at the 9th fret (Ab). Use your middle and index fingers behind the ring finger for support — all three fingers push together.", why: "Supporting fingers behind the bending finger is the #1 technique tip. One finger alone doesn't have enough strength or control. Three fingers working together give you power and pitch accuracy. This is how every professional guitarist bends — the ring finger leads, the others support." },
...

    {
      id: "gs-1-10",
      time: 10,
      title: "Song Study: Dope & Smoke — DOPE LEMON (Am-D-G)",
      type: "guitar",
      metronome: 90,
      chordVoicings: { chords: ["Am", "D", "G"], defaultChord: "Am" },
      tracks: [{ name: "Groove Beat 90", src: "/groove-beat-90.mp3" }],
      what: "Your first complete song. DOPE LEMON's Dope & Smoke is three chords (Am-D-G) at 112 BPM with a mellow, behind-the-beat strumming feel. You'll start at 90 BPM and work up. This song teaches constant arm motion in a real musical context — the lazy, reverb-drenched groove that defines DOPE LEMON's sound.",
      setup: "Guitar. Metronome at 90 BPM (slow it down — original is 112). Optional backing track for groove context.",
      steps: [
        { text: "Play the progression: Am (4 beats) → D (4 beats) → G (4 beats) → G (4 beats). Use all downstrokes first, one per beat. Focus ONLY on clean chord transitions. No fancy strumming yet. Get the changes smooth.", why: "Separating chord changes from strumming pattern is the 'one focal point per exercise' principle. If you try to learn new chords AND a new strum pattern simultaneously, both suffer. Master the changes first." },
        { text: "Once the changes are smooth, add constant arm motion: down-up-down-up on eighth notes. Hit all strums — no misses yet. The arm pendulum provides the groove. Feel how the continuous motion smooths out the chord transitions.", why: "Constant arm motion actually HELPS chord transitions because the rhythmic momentum carries your hand forward. When you're strumming in isolated downstrokes, each transition is a separate event. With continuous motion, transitions happen inside an ongoing flow." },
        { text: "Now shape the strum pattern: on beats 2 and 4, miss the upstroke (arm still swings up, but doesn't hit strings). You've got D-DU-D-DU — this is close to the DOPE LEMON feel. Add a slight accent on the downbeats of 2 and 4.", why: "This pattern creates the laid-back, swaying groove that defines Dope & Smoke. The missing upstrokes on 2 and 4 create rhythmic space — and space is where groove lives. DOPE LEMON's entire aesthetic is about NOT playing every note." },
        { text: "Play the full song loop (Am-D-G-G) with the strum pattern for 8 complete cycles (32 bars). At 90 BPM. No stopping, no restarting. If you miss a chord change, keep the strum going and catch the next one.", why: "Playing through mistakes is a critical musical skill. In a band, if you stop to fix a chord, you derail everyone. If you keep strumming and catch up, nobody notices. This is the difference between practice-room playing and real-world musicianship." },
        { text: "Once comfortable at 90 BPM, try 100 BPM. Then 112 (the real tempo). Don't jump straight to 112 — increase by 10 BPM at a time, and only when the current tempo feels effortless. If you can't get past 100 today, stop. Sleep on it. Try 112 tomorrow.", why: "Speed increases should feel like graduating, not struggling. If 100 BPM feels tight, your brain hasn't fully consolidated the pattern at 90. Sleep consolidation is real — the neural pathways strengthen overnight. Pushing through fatigue teaches sloppy habits." }
      ],
      feel: "This should feel lazy and warm — like strumming on a porch at sunset with nowhere to be. DOPE LEMON's sound is the opposite of urgent. The groove should feel slightly behind the beat, like you're dragging your feet through warm sand. If it feels rushed or anxious, slow down.",
      wrong: "If the chord transitions interrupt the strumming flow, you're trying to change chords too precisely. Let the transition be slightly messy — a brief muted moment between chords is natural and actually adds to the groove. If the strum pattern falls apart every time you change chords, go back to all-downstroke changes until the transitions are automatic, then re-add the pattern.",
      sarah: "Gene, Angus Stone (DOPE LEMON) has this genius quality — he plays simple chords with a strumming feel that sounds like he just woke up from a nap. That's not laziness; it's intentional behind-the-beat placement. His trick is the major→minor oscillation (D-Dm, G-Gm) which you'll explore in later levels. For now, just the Am-D-G with that sleepy, reverb-drenched vibe. This is your sound world — warm, analog, golden hour. Three chords and truth.",
      levelUp: "Can play the Am-D-G progression (Dope & Smoke) with constant arm motion strum pattern at 100+ BPM for 8 cycles without stopping. Chord transitions don't interrupt the strumming flow."
    },
...

    {
      id: "gs-1-11",
      time: 10,
      title: "Song Study: Real Love Baby — Father John Misty (D-Em-G)",
      type: "guitar",
      metronome: 80,
      chordVoicings: { chords: ["D", "Em", "G"], defaultChord: "D" },
      what: "Your second song uses three chords you already know (D-Em-G) but in a different order and feel from Dope & Smoke. Father John Misty's Real Love Baby is 102 BPM with hammer-on embellishments and a more driving, upbeat energy. This teaches you that the same chords in a different order with a different feel create entirely different music.",
      setup: "Guitar. Metronome at 80 BPM (slower than the 102 BPM original). Build up.",
      steps: [
        { text: "Play the progression: D (4 beats) → Em (4 beats) → G (4 beats) → G (4 beats). All downstrokes first. Notice how D→Em shares the middle finger on the same fret? That's your anchor. And Em→G shares the ring finger at the 3rd fret. Use these anchors.", why: "Identifying anchor fingers in a NEW progression reinforces the concept. Every song you learn should start with the question: 'Which fingers stay planted between these chords?' This becomes automatic over time." },
        { text: "Add constant arm motion. This time, try a slightly different pattern than Dope & Smoke: D-DU-DU-DU — steady eighth-note down-ups with an accent on beat 1. This gives a more driving, upbeat feel that matches Father John Misty's energy.", why: "Different strum patterns over the same arm motion demonstrates the principle: the engine (constant arm motion) is always the same — only the pattern of hits and misses changes. Two songs, two feels, same fundamental technique." },
        { text: "Add a hammer-on embellishment to the D chord: strum D, then hammer your pinky onto the high E string 3rd fret (creating a Dsus4 briefly). Lift the pinky and you're back to D. This ornament adds a sparkle that's characteristic of the song.", why: "Hammer-ons add movement and life to static chord strumming. The Dsus4→D hammer-on is one of the most common guitar embellishments — it appears in thousands of folk, pop, and rock songs. Learning it on D gives you a tool you'll use on other chords too." },
        ...
        { text: "Now play Dope & Smoke (Am-D-G) for 4 cycles, then immediately switch to Real Love Baby (D-Em-G) for 4 cycles. Feel how the SAME chords (D and G appear in both) create different musical worlds depending on context. That's the power of chord order and feel.", why: "Switching between songs using overlapping chords teaches harmonic context — D after Am feels different from D after G. You're learning that chords aren't just shapes; they're characters in a story, and their meaning changes depending on what comes before and after." }
      ],
      ...
      wrong: "If the hammer-on sounds weak or doesn't ring, you're not hammering hard enough. The fretting finger should snap down like a tiny hammer — firm and fast. If it mutes the string, make sure your other fingers aren't shifting when you add the pinky. If the strum pattern sounds identical to Dope & Smoke, focus on the accent placement — beat 1 accent for this song, beats 2 and 4 for DOPE LEMON.",
      sarah: "Gene, Father John Misty is a master of simple chords with emotional delivery — Real Love Baby is literally D-Em-G and it's one of the most joyful songs in your rotation. The lesson here is that you don't need complicated chords to make great music. FJM's trick is conviction — he plays three chords like he means every single one. That conviction comes from the strum feel and the hammer-on embellishments, not from chord complexity.",
      levelUp: "Can play D-Em-G (Real Love Baby) at 100 BPM with constant arm motion, hammer-on embellishment on D, and smooth transitions. Can switch between this song and Dope & Smoke without stopping."
    },
...

    {
      ...
      feel: "This should feel like a conversation with a friend who happens to be a backing track. You say something short, they respond with groove, you reply. Relaxed, unhurried, with plenty of comfortable silence. If it feels frantic or you feel pressure to 'fill the space,' play fewer notes. The space IS the music.",
