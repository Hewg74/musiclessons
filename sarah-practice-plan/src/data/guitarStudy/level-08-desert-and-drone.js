...
export const level8 = {
  ...
  exercises: [
...

    {
      ...
      pitchContour: true,
      what: "Play a note over the drone. Close your eyes. Audiate a target interval (e.g., the note a major 3rd above, or the sus 2nd below). Visualize where that note lives on the fretboard — see the fret, the string, your finger landing. Then play it and check. This builds the trimodal map that connects three systems: sound (what you hear) → location (where it lives) → motor action (how to get there). Desert blues guitarists navigate by ear, not by sight.",
      setup: "Drop D tuning. Drone on A. Fretboard diagram available but eyes closed during audiation. Pitch detector on for verification.",
      steps: [
        { text: "Play A on the D string (7th fret). Let it ring over the drone. Now close your eyes. Audiate the note B — one whole step above A. Hear it in your mind. Visualize where B lives on the fretboard: D string, 9th fret. See your finger reaching for it. When the image is vivid, play it. Open eyes — check the fretboard diagram and pitch detector.", why: "This exercise builds the trimodal map that Pascual-Leone 1995 showed reshapes motor cortex: auditory image → spatial visualization → motor execution. Each modality reinforces the others. Desert blues players navigate the neck by sound, not by looking — this is how they do it." },
        { text: "Next target: play B (9th fret, D string). Close eyes. Audiate D — a minor 3rd above B. Visualize D on the G string (7th fret) or the D string (12th fret). Choose one position mentally, then play it. Were you right? Try both positions across 3 reps.", why: "Multiple positions for the same pitch builds a flexible mental map. A guitarist who can only find D in one place is locked into position-based thinking. A guitarist who sees D in multiple locations can navigate freely — which is what improvisation demands." },
        { text: "Full sus pentatonic chain: start on A, audiate the next note up (B), visualize its position, play it. From B, audiate D, visualize, play. From D, audiate E, visualize, play. From E, audiate G, visualize, play. Close eyes for audiation → open to check → close for next rep. Walk up the entire scale this way. As you audiate each target note, notice WHERE in your body the imagined vibration sits. The body address helps you map pitch to fretboard: low notes = chest vibration = bass strings/lower frets, high notes = mask and forehead vibration = treble strings/higher frets. The somatic map and the fretboard map run in parallel — both rise together.", why: "Chaining audiation across the full scale builds a connected internal map rather than isolated note-to-note jumps. Adding the somatic layer creates a three-way map: sound (what you hear) → body (where you feel it) → fretboard (where it lives on the neck). By the end, you've walked A-B-D-E-G with every note heard, felt, and then played. Zamorano 2025 showed body awareness predicts pitch accuracy (R²=0.41) — the body-location channel gives you an additional verification signal that the audiated pitch is correct before your fingers commit to a fret position." },
        { text: "Descending challenge: start on high G. Close eyes, audiate E (a minor 3rd down). Visualize its fretboard position. Play it. Continue down: E→D→B→A. Descending is harder because your ear naturally 'falls' instead of targeting. If you overshoot, note which intervals are weakest.", why: "Descending audiation is harder for most players because gravity makes pitch discrimination less precise going down. Training it specifically builds symmetric inner hearing — equally accurate ascending and descending, which transforms improvisation from 'climbing a ladder' to 'freely navigating space.'" },
        { text: "Free exploration: drone on A, 3 minutes. Before EVERY note you play, close eyes, audiate the target, visualize the fretboard position, then play. Accept that this will be extremely slow — maybe 8-10 notes in 3 minutes. That's correct. The slowness IS the training. Each note was heard, visualized, and then executed.", why: "Aarhus 2021 found that combined imagery + singing + playing equals full learning in 1/3 the time. The slowness of audiating every note is a feature, not a bug. Desert blues is patient music — there's no rush. Every note you play in this exercise was imagined first, which builds the neural pathways that eventually make audiation instantaneous." }
      ],
      feel: "This should feel meditative and precise — like archery with your ears. Each note is an intention, a visualization, and then an action. You're predicting both the sound AND its physical location on the fretboard — two predictions converging on one note. The desert drone grounds everything. When the audiated note matches the played note, there's a deep satisfaction — like an arrow hitting its target. Each rep sharpens your prediction model. The slowness is the practice.",
      wrong: "If you're playing notes without audiating them first, you've reverted to finger-pattern navigation. Stop, close eyes, and hear the next note BEFORE touching the string. If you can't visualize the fretboard with eyes closed, spend a minute staring at the fretboard diagram, then close your eyes and try to recall it — the visual memory builds with practice. If the audiated pitch is consistently wrong, play the note first, remove it, hold the memory for 5 seconds, then re-audiate.",
      sarah: "Gene, Tinariwen guitarists learn by ear around campfires — no fretboard diagrams, no tabs. They hear a phrase and find it. This exercise builds that same skill: hearing a note internally, seeing where it lives, and reaching for it with confidence. Your contemplative nature is an asset here. This is meditation applied to the fretboard — presence, intention, action. When this clicks, you'll start reaching for notes you hear in your head instead of notes your fingers know.",
      metronome: 60,
      recorder: true,
    ...
    },
    {
      ...
      steps: [
        { text: "Let the drone ring. Feel it vibrating continuously through the guitar body into your chest — a warm, constant foundation that lives in your ribcage. In the silence above it, hear a melody arise internally — 4 notes from the sus pentatonic (A-B-D-E-G). Don't reach for the guitar yet. Wait until the phrase feels vivid — until you could sing it, until you can feel where each note lives on the fretboard. Then play it. This is your motif, born from audiation, not from finger patterns.", why: "Desert blues is inner-hearing music — the phrase exists in the player's mind before it reaches the strings. The drone vibrating against your chest is your physical anchor — every melody note you hear internally is heard in RELATION to that vibration. Audiating the motif before playing it ensures the phrase has a melodic identity independent of your finger habits. Pascual-Leone 1995 showed that mental rehearsal alone reshapes motor cortex maps." },
        { text: "Repeat your motif 16 times. Same notes, same rhythm, same dynamics. No variation. Feel how the repetition sinks deeper into your body with each pass — by repetition 8, your fingers know the phrase without thought, and by repetition 12, the drone's vibration against your chest and the melody's movement under your fingertips merge into one continuous physical experience.", why: "Tinariwen's hypnotic quality comes from timbral consistency plus groove — the same phrase played with the same tone and touch creates a trance state that variation would destroy. Neuroscience research on musical entrainment shows that steady repetition synchronizes brainwave patterns with the rhythm, producing a meditative state after roughly 8-12 cycles. The body-level merger of drone and melody is the physical manifestation of that entrainment." },
    ...
    },
    {
      ...
      steps: [
        ...
        { text: "Play for 5 minutes. Keep the drone pattern absolutely steady. Before each melody change, let the drone ring and hear the new note arise internally — wait until it feels vivid, inevitable. Then play it. Change one note every 30 seconds. The drone is the earth; the melody is the wind; and audiation is the silence between them where the next note is born.", why: "The discipline of maintaining the drone while developing the melody is the technical challenge. Audiating each new note over the drone before playing it trains the trimodal connection: sound (hear the interval against the drone) → location (visualize the fret) → action (play it). This inner-hearing approach to melody development is how Tinariwen navigate communal improvisations — each player hears their part in relation to the drone before contributing." }
      ],
    ...
    },
    {
      ...
      wrong: "If the bend goes all the way to C, you've overshot — that's a regular half-step bend. If it doesn't move at all, you need more pressure. The quarter-tone is a very specific, very small movement — the awkwardness of landing between notes is your hand learning a COMPLETELY new precision, like learning to whisper after only knowing how to speak at full volume. Use the 8th fret pitch as your ceiling — stop halfway there. Most beginners under-bend at first, then over-correct — aim for the middle.",
    ...
    },
    {
      ...
      tracks: [{ name: "Desert Blues 75 BPM", src: "/desert-blues-75.mp3" }],
      steps: [
        { text: "Put on Desert Blues 75. Start with just the open D drone. 4 bars of only the bass note. Feel the low D vibrating through the guitar body into your chest — a deep, warm pulse that anchors everything. Let the room fill with this single note. Your body is the resonating chamber.", why: "Starting with the drone alone establishes the physical foundation. The chest vibration from the low D is your anchor for the next 15 minutes — every melody note will be felt in relation to this bass resonance." },
        { text: "Add a simple 3-note sus pentatonic melody over the drone. Repeat it for 3 minutes. Let it become hypnotic. Feel the difference between notes that sit WITH the drone (consonant — your body relaxes, the vibrations merge) and notes that pull AGAINST it (dissonant — your body tenses slightly, the vibrations compete). Don't vary it — commit to the repetition.", why: "The opening melody sets the tone for the entire 15 minutes. The body-level feedback — relaxation on consonance, subtle tension on dissonance — is a faster signal than conscious analysis. This is the embodied version of drone-based improvisation." },
        { text: "Minutes 3-8: gradually evolve. Before each note change, let the drone ring in silence. Close your eyes. The drone vibrating against your chest is your anchor — from that physical foundation, hear the melody forming before your fingers move. Don't search for the next note; let it arise. When it's vivid in your inner ear and you can feel where it lives on the fretboard, play it. Change one note every 90 seconds. Add quarter-tone bends — feel the string resist under your fingertip as you push into the microtonal space between frets. Move between registers while keeping the drone constant.", why: "Desert blues development is glacial and audiation-led. The drone vibrating against your chest provides a physical reference point for every melodic choice. Letting notes arise internally over that felt foundation is the Tinariwen way — the phrase reveals itself through patience, not through searching. Aarhus 2021 showed that combined imagery + playing creates equal learning in 1/3 the time." },
      ...
      feel: "A 15-minute desert session should feel like a journey through a vast, still landscape. When it's working, you lose track of time. The repetition becomes meditative. You hear overtones and harmonics you never noticed. Before you start: what will you listen for in the drone's overtones? After you finish: what surprised you about the 15 minutes?",
    ...
    },
    {
      ...
      tracks: [{ name: "Desert Blues 75 BPM", src: "/desert-blues-75.mp3" }],
      steps: [
        { text: "Start on A (your root). Set a timer for 1 minute. For 60 full seconds, A is your ONLY pitch. Vary everything else: rhythm (long sustains, short bursts, silence, syncopation), dynamics (whisper to full attack), articulation (clean, muted, bent slightly, harmonics). How much music can you make with one note?", why: "One note per minute sounds impossible until you realize how much musical territory exists beyond pitch. Rhythm, dynamics, articulation, and silence are four independent dimensions of expression. With one pitch, all four get your full attention. This is also audiation training in disguise — when your next note is 60 seconds away, you have no choice but to hear it coming. The constraint forces pre-hearing (CLA, Gray 2018: constraining output forces the auditory-cognitive system to lead)." },
        { text: "Minute 2: change to B (the 2nd — the desert note). Feel the string under your fingertip at a new fret — a different physical sensation, a different tension. Explore B for 60 seconds with the same depth. Feel how B against the A drone creates a different space in your body — the vibrations no longer match, creating a floating quality that lives between your chest and your throat. More tension, more ambiguity, more desert.", why: "When a new pitch arrives after 60 seconds of constraint, it sounds monumental and the body registers the shift — the drone's chest vibration interacts differently with the new pitch. The patience makes the change meaningful. This is the same principle as Tinariwen's hypnotic repetition, taken to its logical extreme." },
