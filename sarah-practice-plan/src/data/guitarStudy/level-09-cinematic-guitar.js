  ...
  exercises: [
    {
      ...
      steps: [
        { text: "Position your right hand over the sound hole. Let your thumb (P) hang naturally over the bass strings (E, A, D — strings 6, 5, 4). Your index (I) rests on the G string (3), middle (M) on B string (2), ring (A) on high E string (1). Don't curl your fingers — let them hang in a relaxed arc.", why: "PIMA assignment is the foundation of classical and fingerstyle guitar. Each finger 'owns' its string, so you never have to think about which finger to use. Chet Atkins built his entire career on this one principle: thumb and fingers as independent voices. The thumb handles bass, the fingers handle melody — two instruments from one guitar." },
        { text: "THUMB FIRST: Before adding any fingers, play ONLY with your thumb (P). On Am, alternate between the A string (5th) and the D string (4th) — bass notes only. P on A, P on D, P on A, P on D. Quarter notes at 60 BPM. Do this for 1 full minute until the thumb is completely automatic.", why: "Chet Atkins and Mark Knopfler both insist: the thumb must be on autopilot before you add fingers. Motor learning research confirms this — combining two sub-skills requires each one to be automated first. If your thumb still needs conscious attention, adding fingers creates cognitive overload. One minute of thumb-only practice saves you hours of frustration later." },
        { text: "ADD INDEX: Keep the thumb alternating A-D strings. Now add your index finger (I) plucking the G string (3) between thumb hits. Pattern: P(A string)-I(G string)-P(D string)-I(G string). Two voices now — bass and one melody note. 1 minute at 60 BPM.", why: "Adding one finger at a time is the research-backed sequence. Your thumb keeps its autopilot pattern; the index adds a single new element. If the thumb pattern breaks when you add index, the thumb isn't automated yet — go back to thumb-only." },
        { text: "ADD MIDDLE AND RING: Now the full PIMA. Thumb on A string (5th), then index on G (3), middle on B (2), ring on high E (1). One note at a time: P-I-M-A. Repeat 10 times slowly at 60 BPM.", why: "With the thumb already locked in, adding middle and ring is surprisingly easy — your brain only has to coordinate the three fingers, not all four digits from scratch." },
        { text: "ROLLING PATTERN: P-I-M-A-M-I (thumb, index, middle, ring, middle, index). This creates a continuous, flowing sound — a harp-like cascade from a single chord. 60 BPM, one note per click.", why: "The rolling pattern is the basis of most fingerpicked accompaniment. Mark Knopfler's style was born from exactly this — fingerpicking late at night without an amp, the constraint forcing the technique. The motion should feel like a wave — out and back." },
        { text: "Practice on three chords: Am (P on 5th string A), C (P on 5th string C), G (P on 6th string G). The thumb moves between bass strings while the fingers stay on strings 3-2-1. Keep each chord for 2 bars. Smooth transitions — no gap between chords.", why: "Changing chords while maintaining the fingerpicking pattern is the coordination challenge. Your left hand moves; your right hand stays constant. This independence — what Chet Atkins called 'the boom-chick' — is the key skill that makes one guitar sound like two." }
      ],
      ...
      metronome: 60,
      levelUp: "Play the P-I-M-A-M-I rolling pattern on Am, C, and G at 60 BPM with clean note separation, even volume across all fingers, and smooth chord transitions. The thumb alternates bass strings automatically without conscious thought. No buzzing, no missed strings."
    },
    {
      id: "gs-9-2",
      time: 10,
