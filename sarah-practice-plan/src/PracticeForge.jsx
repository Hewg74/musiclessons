...
// ═══════════════════════════════════════════
// ─── CompactDroneWheel ───
// Merges two Tier-1 tools into a single premium object:
//   (1) sustained root-tone drone player (modal: root + perfect fifth across 3 octaves)
//   (2) miniature circle of fifths showing which notes are in the current scale
//
// Layout (per design-forge spec): 110px dial on the left, 1px vertical divider,
// info column on the right. Dial holds a thin circle-of-fifths orbit track, 12 dots
// positioned clock-style, and a 40px gold play/stop button at dead center. The
// root dot is larger and gets a gentle pulse when playing. When playing, the whole
// dial slowly rotates (60s) and the center button counter-rotates to stay upright.
// prefers-reduced-motion halts all rotation and shows a static glow instead.
//
// Audio is self-contained — a minimal Tone.PolySynth with triangle voices playing
// root+fifth+octave. We do NOT depend on the big DroneGenerator from JungleTools
// because its preset/cycle/texture/BPM knobs are overkill for a practice reference.
// ═══════════════════════════════════════════
const DRONE_CHROMATIC = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
function droneNoteIndex(note) {
  if (!note) return -1;
  const n = note
    .replace('♭', 'b')
    .replace(/^Db$/, 'C#')
    ...
    .replace(/^Ab$/, 'G#')
    .replace(/^Bb$/, 'A#');
  return DRONE_CHROMATIC.indexOf(n);
}

// Parse a chord symbol ('Am', 'C', 'F#m', 'Dsus4', 'Bbdim') into a root-heavy pad
// voicing. Simplified fork of JungleTools' parseChordToNotes — same philosophy
// (root tripled across octaves so it dominates, color tones spread up) but tailored
// for drone-reference context (no 7ths, no maj7, no cycle mode).
// Returns an array of Tone.js note names like ["A2", "A3", "E3", "A4", "C5"].
function droneParseVoicing(chordStr, baseOct = 3) {
  const match = String(chordStr || '').trim().match(/^([A-G][#b♭]?)(.*)$/i);
  if (!match) return [];
  let root = match[1].toUpperCase().replace('♭', 'b');
  const q = match[2].toLowerCase();

  // Normalize flat spellings to sharps so Tone.js is happy with them
  const normalize = (n) => {
    if (n === 'Db') return 'C#';
    if (n === 'Eb') return 'D#';
    if (n === 'Gb') return 'F#';
  ...
  const idx = DRONE_CHROMATIC.indexOf(nroot);
  if (idx === -1) return [];

  const getNote = (intervalSemitones, octOffset) => {
    const i = (idx + intervalSemitones) % 12;
    const wrap = Math.floor((idx + intervalSemitones) / 12);
    return `${DRONE_CHROMATIC[i]}${baseOct + octOffset + wrap}`;
  };

  // Quality detection — order matters (check maj7 before m, sus before bare)
  let third = 4;    // default major 3rd
  let fifth = 7;    // default perfect 5th
  if (q.match(/^m(?!aj)/) || q.includes('min')) third = 3;     // minor 3rd
  if (q.includes('dim')) { third = 3; fifth = 6; }              // diminished: minor 3rd + tritone
  if (q.includes('aug')) { third = 4; fifth = 8; }              // augmented: major 3rd + minor 6th
  if (q.includes('sus2')) third = 2;                            // sus2: whole step instead of 3rd
  if (q.includes('sus4')) third = 5;                            // sus4: perfect 4th instead of 3rd

  // Root-heavy pad voicing: root tripled so it dominates, fifth holds the middle,
  // third spreads up high (perceptually quieter at same gain). This mirrors how
  // real tanpuras / shrutis balance their drone.
  return [
    getNote(0, -1),    // Sub root (deep)
    getNote(0, 0),     // Main root
    getNote(fifth, 0), // Fifth (mid)
    getNote(0, 1),     // Root octave
    getNote(third, 1), // Third (spread up, air)
  ];
}

// Persist drone preferences across sessions — users dial in their preferred
// octave + volume once and expect it to stick.
const DRONE_PREFS_KEY = 'practiceforge-drone-prefs';
function loadDronePrefs() {
  try {
    const raw = localStorage.getItem(DRONE_PREFS_KEY);
    return raw ? JSON.parse(raw) : {};
  } catch { return {}; }
}
function saveDronePrefs(prefs) {
  try { localStorage.setItem(DRONE_PREFS_KEY, JSON.stringify(prefs)); } catch {}
}

function CompactDroneWheel({ T, rootKey, scaleNotes, scaleName }) {
  const [playing, setPlaying] = useState(false);
  // Persisted preferences — octave (2-5) and volume (-30 to 0 dB)
  const [octave, setOctave] = useState(() => {
    const p = loadDronePrefs();
    return typeof p.octave === 'number' ? p.octave : 3;
  });
  const [volume, setVolume] = useState(() => {
    const p = loadDronePrefs();
    return typeof p.volume === 'number' ? p.volume : -12;
  });
  ...
  // Visual geometry constants (from design-forge spec §4)
  const containerSize = 110;
  const trackSize = 90;
  const cx = containerSize / 2;
  const cy = containerSize / 2;
  const orbitR = trackSize / 2;

  // Keep refs in sync with state (so async handlers read live values)
  ...
  // Persist preferences
  useEffect(() => {
    saveDronePrefs({ octave, volume });
  }, [octave, volume]);

  // Live volume updates during playback — no re-trigger needed, just ramp the gain
  useEffect(() => {
    if (playing && gainRef.current) {
      try {
        gainRef.current.gain.cancelScheduledValues(Tone.now());
        gainRef.current.gain.rampTo(Tone.dbToGain(volume), 0.15);
      } catch {}
    }
  }, [volume, playing]);

  // Stop drone when root key changes (new card drawn)
  useEffect(() => {
    if (playingRef.current) {
      try {
        if (gainRef.current) {
          gainRef.current.gain.cancelScheduledValues(Tone.now());
          gainRef.current.gain.rampTo(0, 0.12);
        }
        const t = setTimeout(() => {
  ...
  // Re-trigger voicing when octave changes while playing
  useEffect(() => {
    if (!playingRef.current || !synthRef.current || synthRef.current.disposed) return;
    try {
      const notes = droneParseVoicing(rootKey, octave);
      if (notes.length === 0) return;
      synthRef.current.releaseAll();
      // Tiny delay so the release envelope doesn't step on the attack
      setTimeout(() => {
        try {
  ...
  useEffect(() => {
    return () => {
      try {
        if (gainRef.current) {
          gainRef.current.gain.cancelScheduledValues(Tone.now());
          gainRef.current.gain.rampTo(0, 0.05);
        }
        setTimeout(() => {
          try { synthRef.current?.releaseAll(); } catch {}
  ...
  const togglePlay = useCallback(async () => {
    ...
    // Start audio context (user gesture unlocks it)
    ...
    if (!synthRef.current || synthRef.current.disposed) {
      ...
      synthRef.current = new Tone.PolySynth(Tone.Synth, {
        ...
        envelope: { attack: 1.2, decay: 0.3, sustain: 1, release: 1.4 },
      }).connect(gainRef.current);
    }

    // Proper root-heavy pad voicing — respects major/minor/dim/aug/sus from the
    // chord symbol. Card passes e.g. 'Am' for minor scales, 'A' for major.
    const droneNotes = droneParseVoicing(rootKey, octaveRef.current);
    if (droneNotes.length === 0) {
      setPlaying(false);
      return;
    }

    try {
      synthRef.current.triggerAttack(droneNotes);
      gainRef.current.gain.rampTo(Tone.dbToGain(volumeRef.current), 0.4);
      setPlaying(true);
    } catch {}
  }, [playing, rootKey]);

  // Normalize rootKey for comparison with CIRCLE_OF_FIFTHS entries
  const cleanRoot = String(rootKey || 'A').replace(/m$/, '');
  const rootNormalized = normalizeNote(cleanRoot);
  const isMinor = /m(?!aj)/.test(String(rootKey || ''));
  ...
  return (
    <div
      style={{
        ...
        boxShadow: playing ? `0 2px 14px rgba(212, 163, 115, 0.12)` : 'none',
      }}
    >
      {/* Dial — rotates 60s when playing (reduced-motion honored) */}
      <div
        aria-hidden="true"
        style={{
          ...
          animation: playing && !reducedMotion
            ? 'forgeDroneOrbit 60s linear infinite'
            : 'none',
        }}
      >
        {/* Track circle — very subtle, sets the "orbit" metaphor */}
        <div style={{
          position: 'absolute',
          top: (containerSize - trackSize) / 2,
          left: (containerSize - trackSize) / 2,
        ...
        {/* 12 orbit dots — scale notes colored + filled, non-scale outlined faint */}
        {CIRCLE_OF_FIFTHS.map((note, i) => {
          ...
          const isRoot = normalized === rootNormalized;
          const color = getColorForNote(note) || T.textMuted;
          const size = isRoot ? 10 : inScale ? 6 : 4;
          const pulseAnim = isRoot && playing
            ? (reducedMotion ? 'none' : 'forgeDronePulse 2.8s cubic-bezier(0.4, 0, 0.2, 1) infinite')
            : 'none';
          const staticGlow = isRoot && reducedMotion && playing
            ? `0 0 10px ${color}66`
            : inScale ? `0 1px 2px ${color}44` : 'none';
          return (
            <div
              key={note}
              style={{
                ...
                borderRadius: '50%',
                background: inScale ? color : 'transparent',
                border: inScale ? 'none' : `1px solid ${T.textMuted}`,
                borderColor: inScale ? undefined : `rgba(184, 178, 171, 0.4)`,
                boxShadow: staticGlow,
                zIndex: isRoot ? 10 : 1,
                animation: pulseAnim,
                transform: isRoot && reducedMotion && playing ? 'scale(1.1)' : undefined,
        ...
        {/* Center play/stop — counter-rotates to stay upright */}
        <button
          onClick={togglePlay}
          aria-label={playing ? 'Stop drone' : 'Play drone'}
          title={playing ? 'Stop drone' : 'Play drone'}
          style={{
            position: 'absolute',
            left: (containerSize - 40) / 2,
            top: (containerSize - 40) / 2,
            ...
            animation: playing && !reducedMotion
              ? 'forgeDroneOrbitCounter 60s linear infinite'
              : 'none',
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.background = T.goldDark;
          }}
          onMouseLeave={(e) => {
        ...
        >
          {playing
            ? <Pause size={16} color="#fff" strokeWidth={1.5} />
            : <Play size={16} color="#fff" strokeWidth={1.5} style={{ marginLeft: 2 }} />}
        </button>
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{
          ...
          gap: 14, marginBottom: 5, flexWrap: 'wrap',
        }}>
          <div style={{
            fontFamily: T.serif,
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            {scaleNotes.map((note, i) => {
              const color = getColorForNote(note) || T.textMed;
              return (
                <span
                  key={i}
                  style={{
        ...
        <div style={{
          ...
          {scaleName
            ? `${scaleName.toLowerCase()} ${isMinor ? 'minor triad' : 'triad'} — play before improvising`
            : 'root-tone reference — play before improvising'}
        </div>

        {/* Secondary control row — octave ± and volume slider.
            Thin, inline, doesn't break the premium feel. Persisted across sessions. */}
        <div style={{
          ...
          {/* Volume slider cluster */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '1 1 120px', minWidth: 120 }}>
            <span style={{
              fontSize: 9, fontWeight: 700, color: T.textMuted,
            ...
            <span style={{
              ...
              color: T.textLight, fontVariantNumeric: 'tabular-nums',
            }}>
              {volume} dB
            </span>
          </div>
    ...
    >
      ...
      {currentCard && (
        <>
          ...
          {/* Compact drone + circle-of-fifths — merged Tier-1 object. Replaces
              the old DroneGenerator row AND the separate color wheel tool.
              See CompactDroneWheel component above for design-forge spec details. */}
